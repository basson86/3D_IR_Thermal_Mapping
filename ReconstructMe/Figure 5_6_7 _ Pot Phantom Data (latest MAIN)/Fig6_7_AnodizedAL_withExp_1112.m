...
% plot the normalized temerperature differences with the simulated
% emissivity from non-dielectric, dielectric, and cosine models
figure(3);
[AX,H1,H2] = plotyy(theta_half, Avg_DelT_Emis(:,center:1:right_B),phi*180/pi, rho_phi_ND,'plot');
hold on;
set(H1,'LineStyle','*');

[AX,H1,H3] = plotyy(theta_half, Avg_DelT_Emis(:,center:1:right_B),phi*180/pi, rho_phi_D,'plot');
hold on;
set(H1,'LineStyle','*');

[AX,H1,H4] = plotyy(theta_half, Avg_DelT_Emis(:,center:1:right_B),phi*180/pi, 1-E0*cos(phi),'plot');
set(H1,'LineStyle','*');

set(get(AX(1),'Ylabel'),'String','Ts-T(\theta)/Ts-Ta') ;
set(get(AX(2),'Ylabel'),'String','1-\epsilon(\theta)') ;
set(H2,'LineStyle','-');
set(H3,'LineStyle','square');
set(H4,'LineStyle','-.');
legend([H1,H2,H3,H4],'Normalized temperature decay detected in experiment','1-\epsilon(\theta) by non-dielectric model','1-\epsilon(\theta) by dielectric model','1-\epsilon(\theta) by cosine model');
xlabel('Viewing Angle \theta (degree)');
grid on;


%% plot the mean temperature differences (not the normalized form) generatred by viewing angle
mean_Ts=mean([34,36,38,40,42,44]);
Ta=22.63;
shifting_zero=Avg_DelT_Emis(:,center)*(mean_Ts-22.63);
figure(4);
plot(theta_half, Avg_DelT_Emis(:,center:1:right_B)*(mean_Ts-22.63),'b*');
xlabel('Viewing Angle \theta (degree)');
ylabel('\Delta T(^o C)');
legend('Temperature decay detected in experiment');
ylim([4 16]);
grid on;
