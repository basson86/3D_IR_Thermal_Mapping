...
figure;
imagesc(SS_T(TempPos_t(1,1):TempPos_t(1,2),TempPos_t(2,1):TempPos_t(2,2)));
colormap('jet');
colorbar;
...
% Select the range of skin for mean & variation computation
figure, imagesc(mat2gray(ROI_temp_1));
colormap('jet');
title('please select the boundary of ROI for computation');
Mean_pos= ginput(2);
Mean_Pos=[Mean_pos(:,2),Mean_pos(:,1)]';
c_to_meanPos= [Mean_Pos(1,:)-[center(1),center(1)]; Mean_Pos(2,:)-[center(2),center(2)]];
...
for i= initial:final
    ...
    Mean_Pos(1,:)= c_to_meanPos(1,:) + TemplateData(1).p(5);
    Mean_Pos(2,:)= c_to_meanPos(2,:) + TemplateData(1).p(6);
    ...
    for n= 1: size(Lesion_B_IR_mapped,2);
      LB_s(n,:)=TemplateData(1).p(5:6) + c_to_LB(n,:)-C1-Mean_Pos(:,1)';    
    end
    ...
    M_ROI_1= ROI_temp_1(Mean_Pos(1,1):Mean_Pos(1,2),Mean_Pos(2,1):Mean_Pos(2,2));
    M_ROI= ROI_temp_i(Mean_Pos(1,1):Mean_Pos(1,2),Mean_Pos(2,1):Mean_Pos(2,2));
    %mean_temp= mean(mean(M_ROI_1));
    ...
    %mesh(Var_ROI);
    imagesc(Var_ROI, [0,1.6]);
    colorbar;
    hold on;
    %plot(LB_s(:,2), LB_s(:,1),'r.','MarkerSize',10);
    
% % Show the movie frame
%     if(i==initial), figure, handle_imshow=imagesc(ROI_temp_i_d, [0 TempD_scale]);
%        colormap('jet');   hold on;
%     else
%         for t=1:length(TemplateData); end; 
%         clims = [0 TempD_scale];
...
%     for t=1:length(TemplateData)
%           h(t)=plot(lesion_p_d{i-initial+1}(1:point_num,2),lesion_p_d{i-initial+1}(1:point_num,1),'b.','MarkerSize',5,'MarkerFaceColor',cmap(round(t*255/length(TemplateData))+1,:)); 
%           h1(t)=plot(healthy_p_d{i-initial+1}(1:point_num,2),healthy_p_d{i-initial+1}(1:point_num,1),'m.','MarkerSize',5,'MarkerFaceColor',cmap(round(t*255/length(TemplateData))+1,:)); 
%           drawnow;
%     end
