...
% stamp='164923';  % practice trial 2012/01/23
% W_file_name='IMG_0367';
% initial= 1;
% final= 5; 
% SSstamp='164923';  % steady state image for comparison
% SS_N= 1;

stamp='172539';  % practice trial 2012/01/23
W_file_name='IMG_0371';
initial= 44;
final= 144; 
SSstamp='171614';  % steady state image for comparison
SS_N= 1;


%% set the time step to plot the recovery curve
time_step=1;
...
% [CW_t,CIR_S]=Corners_manual_selection(W_file_name,SS_R);
% 
% save('CW.mat','CW');
% save('CIR.mat', 'CIR');
...
% figure, imagesc(SS_T);
% colormap('jet');
% title('please select the boundary of template in the steady state');
% Temp= ginput(2);
% TempPos_t=[Temp(:,2),Temp(:,1)]';
...
[Lesion_B_IR_S]=lesion_mapping_SS(Lesion_B_W,SS_T,CW,CIR_S);
[l_p,h_p,Lesion_B_IR_mapped,CIR_mapped,point_num]=lesion_points_mapping_auto(Lesion_B_W,ImT1,CW,CIR);
%[l_p,h_p,Lesion_B_IR_mapped,CIR_mapped,point_num]=lesion_points_mapping_auto(Lesion_B_W,SS_T,CW,CIR_S);

% load('l_p.mat');
% load('h_p.mat');

%% Display the selected lesion and healthy tissue in the steady state image
[l_p_S,h_p_S]=Show_LP_HP_SS(l_p,h_p,SS_T,CIR,CIR_S); 
...
figure,
plot(0:time_step:(final-initial+1)*time_step-1,Temp_rec_l(:,1),'b*'); 
hold on;
grid on;
plot(0:time_step:(final-initial+1)*time_step-1,Temp_rec_h(:,1),'m.'); 
...
figure;
plot(0:time_step:(final-initial+1)*time_step-1,LT(:,1:point_num),'b*'); 
hold on;
grid on;
plot(0:time_step:(final-initial+1)*time_step-1,HT(:,1:point_num),'m.'); 
