...
for i= initial:final
    ...
    for t=1:length(TemplateData)
          h(t)=plot(lesion_p_d{i-initial+1}(1:point_num,2),lesion_p_d{i-initial+1}(1:point_num,1),'bo','MarkerSize',1.5,'MarkerFaceColor',cmap(round(t*255/length(TemplateData))+1,:)); 
          h1(t)=plot(healthy_p_d{i-initial+1}(1:point_num,2),healthy_p_d{i-initial+1}(1:point_num,1),'mo','MarkerSize',1.5,'MarkerFaceColor',cmap(round(t*255/length(TemplateData))+1,:)); 
          drawnow;
    end
...
% record the temperature curve as the function of time at lesion & healthy
% tissue
for p=1:point_num
