...
stamp='132836';  % patient 039
W_file_name='IMG1';
initial= 13;
final=103; 
SSstamp='132624';  % steady state image for comparison
SS_N= 1;
...
% manually picked point
%[l_p,h_p,Lesion_B_IR_mapped,CIR_mapped]=lesion_points_mapping(Lesion_B_W,ImT1,9);
...
for i= initial:final
    ...
    % fix the lesion & healthy point (without using template-tracking for
    % patient 39
    lesion_p{i-initial+1}(:,1)=l_p(:,2);
    lesion_p{i-initial+1}(:,2)=l_p(:,1);
    healthy_p{i-initial+1}(:,1)=h_p(:,2);
    healthy_p{i-initial+1}(:,2)=h_p(:,1);
...
title(strcat('t=',num2str((i-initial)*2),'s'));
...
hold on;
grid on;
plot(0:2:(final-initial+1)*2-1,Temp_rec_h(:,1),'k.','MarkerSize',10); 
legend('lesion ','healthy tissue ',2);
...
%point_num=1;
figure;
plot(0:2:(final-initial+1)*2-1,LT(:,1:point_num),'g*','MarkerSize',5); 
hold on;
grid on;
plot(0:2:(final-initial+1)*2-1,HT(:,1:point_num),'k.','MarkerSize',5); 
title('Recovery Temperature (9 points)');
xlabel('Time (sec)');
ylabel('Recovery temperature (^C)');
