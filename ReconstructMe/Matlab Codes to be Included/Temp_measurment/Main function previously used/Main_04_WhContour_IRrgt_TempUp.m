...
% stamp='130253';  % practice trial 2012/02/16 -Hand_2_markers_round
% W_file_name='IMG_0446';
% initial= 71;
% final= 190; 
% Update_Frame=120;
% SSstamp='130253';  % steady state image
% SS_N= 1;
% time_step=1;
% Frame_int= 1;
...
% [CWm,CIR_Sm]=Corners_manual_selection_Wh2IR(W_file_name,ImTS);
% [CIR_S,CIR]=Corners_manual_selection_IRs2IR(ImTS,ImT1);
% save('CWm.mat','CWm');
% save('CIR_Sm.mat','CIR_Sm');
...
% [Lesion_IRs,imRs_gray]=lesion_outlines_IRs(ImTS);
...
[l_p,h_p,Lesion_B_IRs_mapped,CIRs_mapped,point_num]=lesion_points_mapping_auto(Lesion_B_W,ImTS,CWm,CIR_Sm);
[l_p,h_p,Lesion_B_IRr_mapped,CIRr_mapped,point_num]=lesion_points_mapping_auto(Lesion_B_IRs_mapped,ImT1,CIR_S,CIR);
...
% Compute the relative position of lesion & healthy tissue w.r.t the
% template center


% Lesion Boundary Mapping 
for k=1:size(Lesion_B_IRr_mapped,2);
c_to_lesion_B_t(:,k)= Lesion_B_IRr_mapped(:,k)- [center(2);center(1);0];
end
% put the row coordinate to the second index
c_to_lesion_B=[c_to_lesion_B_t(2,:);c_to_lesion_B_t(1,:);c_to_lesion_B_t(3,:)];


% Lesion centroid & healthy tissue
...
for i= initial:Frame_int:final
    ...
    % Warping of Lesion Boundary
    lesion_B_coor=M*c_to_lesion_B;
...
%         subplot(1,2,2);
    ...
    for t=1:length(TemplateData)
          ...
          plot(lesion_B_coor(2,:),lesion_B_coor(1,:),'k.','MarkerSize',2,'MarkerFaceColor',cmap(round(t*255/length(TemplateData))+1,:)); 
          drawnow;
    end
