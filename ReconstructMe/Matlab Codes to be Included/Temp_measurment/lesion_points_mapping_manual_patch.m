function[l_p,h_p,Lesion_B_IR_mapped,CIR_mapped,point_num]=lesion_points_mapping_manual_patch(patch_size,Lesion_B_W,imR_ir,CW,CIR,lowT,highT)
...
l_centroid= ginput(1);
% get the probe region of lesion by selecting the "patch_size x patch_size" pixels mask centered
% at its centroid

half_patch= floor(patch_size/2);

[l_x,l_y]= meshgrid(l_centroid(1,1)-half_patch:1:l_centroid(1,1)+half_patch,l_centroid(1,2)-half_patch:1:l_centroid(1,2)+half_patch);
l_p(:,1)= reshape(l_x,(half_patch*2+1)^2,1);
l_p(:,2)= reshape(l_y,(half_patch*2+1)^2,1);
